...
// Slider verilerini çekme
$query = mysqli_query($conn, 'SELECT * FROM slider WHERE status=1 ORDER BY RAND()');
if (!$query) {
    die("Slider sorgusu başarısız oldu: " . mysqli_error($conn));
}
?>
...
<?php elseif (isset($danger) && $danger): ?>
    <div class="alert alert-danger"><?php echo htmlspecialchars($danger); ?></div>
<?php endif; ?>
        ...
        <div class="carousel-inner">
            ...
            while ($slider = mysqli_fetch_array($query)): ?>
                <div class="carousel-item <?php echo $count == 0 ? 'active' : ''; ?>">
                    <img class="w-100" src="upload_slider/<?php echo htmlspecialchars($slider['slider_image']); ?>" alt="Slider Image">
                    <div class="carousel-caption d-flex flex-column align-items-center justify-content-center">
                        <div class="p-3" style="max-width: 700px;">
                            <h6 class="section-title text-white text-uppercase mb-3 animated slideInDown">
                                <?php echo htmlspecialchars($slider['slider_title1'] ?: 'Dəbdəbəli Yaşayış'); ?>
                            </h6>
                            <h1 class="display-3 text-white mb-4 animated slideInDown">
                                <?php echo htmlspecialchars($slider['slider_title2'] ?: 'Brend Oteli kəşf edin'); ?>
                            </h1>
            ...
            if ($count == 0) {
                echo '<div class="alert alert-warning">Slider verisi bulunamadı.</div>';
            }
            ?>
        </div>
